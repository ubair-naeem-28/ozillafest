...
const DEFAULTS = {
  color: '#ff8a3d', // warm festival neon amber/orange matching Ozilla theme
  rounded: 24,
  thickness: 2.5,
  borderSize: 45,
  glow: 85,
...
export default function NeonBorder({
  ...
  const radius =
    ...
    (Math.min(size.w || 200, size.h || 200) / 2)
  ...
  const glowLayer = (key, r, blurPx, opacity) => (
    <div
      key={key}
      style={{
        position: 'absolute',
  ...
  return (
    <div
      ref={rootRef}
      className={`neon-border-overlay ${className}`}
      style={{
        position: 'absolute',
        inset: 0,
        width: '100%',
