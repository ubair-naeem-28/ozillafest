import{j as e,m as f}from"./vendor-motion-DHTbcX3O.js";import{r as u,u as lt,L as he}from"./vendor-react-6MXTWAEx.js";import{b as I}from"./index-BijBq5WR.js";import{o as Re}from"./ozillaProfessionalContent-BsUWdkVC.js";import"./vendor-utils-Bub7y9MC.js";I("/assets/ozilla/talwinder.jpg"),I("/assets/ozilla/talwinder.jpg"),I("/assets/ozilla/Imran-Khan.jpg"),I("/assets/ozilla/Imran-Khan.jpg"),I("/assets/ozilla/Bohemia.jpg"),I("/assets/ozilla/Bohemia.jpg"),I("/assets/ozilla/hassan-raheem.jpg"),I("/assets/ozilla/hassan-raheem.jpg");const ke=[{name:"TALWINDER",isHeadliner:!0,genre:"Indie Pop / Punjabi Fusion",stage:"Main Prism Stage",bpm:"128 BPM",vibe:"Euphoric Night",src:I("/assets/ozilla/talwinder.jpg"),image:I("/assets/ozilla/talwinder.jpg"),offsetY:0},{name:"IMRAN KHAN",isHeadliner:!1,genre:"Punjabi Pop / Urban",stage:"Neon Bass Arena",bpm:"132 BPM",vibe:"High Energy",src:I("/assets/ozilla/Imran-Khan.jpg"),image:I("/assets/ozilla/Imran-Khan.jpg"),offsetY:0},{name:"BOHEMIA",isHeadliner:!1,genre:"Punjabi Rap / Desi Hip Hop",stage:"Underground Stage",bpm:"95 BPM",vibe:"Heavy Bass",src:I("/assets/ozilla/Bohemia.jpg"),image:I("/assets/ozilla/Bohemia.jpg"),offsetY:0},{name:"HASSAN RAHEEM",isHeadliner:!1,genre:"Pop / Indie R&B",stage:"Sunset Stage",bpm:"115 BPM",vibe:"Golden Hour Chill",src:I("/assets/ozilla/hassan-raheem.jpg"),image:I("/assets/ozilla/hassan-raheem.jpg"),offsetY:0}],Te=1.28,ze=.78;function le(r,n,t){return Math.min(t,Math.max(n,r))}function Fe(r){if(!r)return null;if(typeof r=="string")return r||null;const n=r.src||r.image;return typeof n=="string"&&n?n:null}function ct(r){return Fe(r&&typeof r=="object"&&(r.image||r.src)||r)}function dt(r){if(r&&typeof r=="object"&&"offsetY"in r){const n=r.offsetY;return typeof n=="number"&&isFinite(n)?n:0}return 0}function pt({items:r=ke,images:n,slideWidth:t=320,slideHeight:i=440,spacing:l=2.4,direction:y="right",smoothness:w=8.5,radius:E=24,dim:v=5,background:R="transparent",sensitivity:D=5,autoPlay:L=!1,autoPlayInterval:B=3500,style:H,onItemClick:U}){const O=u.useRef(null),_=u.useRef([]),P=u.useRef(0),V=u.useRef(0),[T,z]=u.useState(0),[ie,re]=u.useState(!1),[g,d]=u.useState(0),a=r&&r.length>0?r:n&&n.length>0?n:ke,o=u.useMemo(()=>a.map((s,c)=>({...s,src:ct(s),offsetY:dt(s),name:s.name||`Artist ${c+1}`,genre:s.genre||"Live Performer",stage:s.stage||"Ozilla Arena",bpm:s.bpm||"128 BPM",vibe:s.vibe||"Festival Energy",isHeadliner:!!s.isHeadliner})),[a]),x=T>0&&T<640?Math.min(t,250):T<1024?Math.min(t,290):t,b=T>0&&T<640?Math.round(x*1.36):i,M=x+le(l,0,10)*18,N=.15-le(w,0,10)/10*.11,K=le(v,0,10)/10*.75,F=.4+le(D,0,10)/10*1.1,G=.6+le(D,0,10)/10*1.6,J=y==="left",W=u.useRef({count:o.length,step:M,slideWidth:x,width:T,ease:N,maxScale:Te,minScale:ze,dim:K,loop:!1,flip:J});W.current={count:o.length,step:M,slideWidth:x,width:T,ease:N,maxScale:Te,minScale:ze,dim:K,loop:!1,flip:J};const C=u.useRef({wheelMultiplier:F,dragMultiplier:G,flip:J});C.current={wheelMultiplier:F,dragMultiplier:G,flip:J},u.useEffect(()=>{const s=O.current;if(!s)return;const c=new ResizeObserver(m=>{m[0]&&z(m[0].contentRect.width)});return c.observe(s),z(s.getBoundingClientRect().width),()=>c.disconnect()},[]),u.useEffect(()=>{_.current.length=o.length},[o.length]),u.useEffect(()=>{if(!L||ie||o.length<=1)return;const s=setInterval(()=>{d(c=>{const m=(c+1)%o.length;return P.current=m*M,m})},B);return()=>clearInterval(s)},[L,ie,o.length,M,B]),u.useEffect(()=>{let s=0,c=0;const m=S=>{s=requestAnimationFrame(m);const p=W.current,$=c?Math.min((S-c)/1e3,.1):1/60;if(c=S,!p.count||p.step<=0||p.width<=0)return;const k=(p.count-1)*p.step;P.current=le(P.current,0,k);const Z=1-Math.pow(1-p.ease,$*60);V.current+=(P.current-V.current)*Z;const ee=Math.round(V.current/p.step);ee>=0&&ee<p.count&&d(ee);const fe=(p.width-p.slideWidth)/2,ue=p.width/2;for(let ne=0;ne<p.count;ne+=1){const te=_.current[ne];if(!te)continue;const j=ne*p.step-V.current+fe,A=j+p.slideWidth/2-ue;let h,Y;A>0?(h=Math.min(p.maxScale,1+A/p.width*.45),Y=(h-1)*p.slideWidth*.35):(h=Math.max(p.minScale,1+A/p.width*.5),Y=0);const ce=p.flip?p.width-p.slideWidth-(j+Y):j+Y;if(te.style.transform=`translate3d(${ce}px, -50%, 0) scale(${h})`,p.dim>0&&h<1){const Q=(1-h)/Math.max(.001,1-p.minScale);te.style.filter=`brightness(${Math.max(.35,1-Q*p.dim)})`}else te.style.filter="none"}};return s=requestAnimationFrame(m),()=>cancelAnimationFrame(s)},[]),u.useEffect(()=>{const s=O.current;if(!s)return;const c=m=>{m.preventDefault();const S=Math.abs(m.deltaX)>Math.abs(m.deltaY)?m.deltaX:m.deltaY;P.current+=S*C.current.wheelMultiplier};return s.addEventListener("wheel",c,{passive:!1}),()=>s.removeEventListener("wheel",c)},[]),u.useEffect(()=>{const s=O.current;if(!s)return;let c=null,m=0;const S=k=>{if(c===null){c=k.pointerId,m=k.clientX;try{s.setPointerCapture(k.pointerId)}catch{}}},p=k=>{if(c!==k.pointerId)return;const Z=k.clientX-m;m=k.clientX,P.current+=(C.current.flip?Z:-Z)*C.current.dragMultiplier},$=k=>{if(c===k.pointerId){c=null;try{s.hasPointerCapture(k.pointerId)&&s.releasePointerCapture(k.pointerId)}catch{}}};return s.addEventListener("pointerdown",S),s.addEventListener("pointermove",p),s.addEventListener("pointerup",$),s.addEventListener("pointercancel",$),()=>{s.removeEventListener("pointerdown",S),s.removeEventListener("pointermove",p),s.removeEventListener("pointerup",$),s.removeEventListener("pointercancel",$)}},[]);const X=u.useCallback(s=>{const c=le(s,0,o.length-1);P.current=c*M,d(c)},[o.length,M]),se=u.useCallback(s=>{const c=le(g+(s==="left"?-1:1),0,o.length-1);X(c)},[g,X,o.length]);return e.jsxs("div",{style:{position:"relative",width:"100%",display:"flex",flexDirection:"column",alignItems:"center",margin:"1.2rem 0"},onMouseEnter:()=>re(!0),onMouseLeave:()=>re(!1),children:[e.jsx("div",{style:{display:"flex",alignItems:"center",justifyContent:"center",flexWrap:"wrap",gap:"8px",marginBottom:"1rem",zIndex:10},children:o.map((s,c)=>{const m=c===g;return e.jsx("button",{type:"button",onClick:()=>X(c),style:{padding:"6px 14px",borderRadius:"999px",border:m?"1px solid #ff8a3d":"1px solid rgba(255, 255, 255, 0.12)",background:m?"linear-gradient(135deg, rgba(255, 138, 61, 0.32), rgba(255, 90, 31, 0.22))":"rgba(20, 10, 6, 0.55)",color:m?"#ffbd59":"rgba(255, 255, 255, 0.65)",fontSize:"0.78rem",fontWeight:m?"850":"650",letterSpacing:"0.04em",cursor:"pointer",transition:"all 200ms ease",backdropFilter:"blur(8px)",boxShadow:m?"0 0 16px rgba(255, 138, 61, 0.35)":"none"},children:s.name},s.name)})}),e.jsx("div",{ref:O,className:"smooth-scroll-slider-viewport",style:{position:"relative",width:"100%",height:`${b+50}px`,overflow:"hidden",background:R,cursor:"grab",touchAction:"pan-y",opacity:T>0?1:0,transition:"opacity 0.35s ease",userSelect:"none",...H},children:o.map((s,c)=>{const m=s.isHeadliner,S=c===g;return e.jsxs("div",{ref:p=>{_.current[c]=p},onClick:()=>{X(c),U&&U(s,c)},style:{position:"absolute",top:"50%",left:0,width:`${x}px`,height:`${b}px`,borderRadius:`${E}px`,overflow:"hidden",background:"#130c0a",border:S?"1.5px solid rgba(255, 138, 61, 0.85)":m?"1px solid rgba(255, 138, 61, 0.45)":"1px solid rgba(255, 255, 255, 0.12)",boxShadow:S?"0 26px 65px rgba(0,0,0,0.9), 0 0 35px rgba(255, 90, 31, 0.4)":"0 18px 45px rgba(0,0,0,0.75)",willChange:"transform, filter",transform:"translate3d(0, -50%, 0)",cursor:"pointer",transition:"border-color 250ms ease, box-shadow 250ms ease"},children:[s.src?e.jsx("img",{src:s.src,alt:s.name,draggable:!1,onError:p=>{p.currentTarget.src=I("/assets/prism-auth-visual.jpg")},style:{width:"100%",height:"100%",objectFit:"cover",objectPosition:`50% calc(50% + ${s.offsetY}px)`,display:"block",userSelect:"none"}}):null,e.jsxs("div",{style:{position:"absolute",top:"12px",left:"12px",right:"12px",display:"flex",alignItems:"center",justifyContent:"space-between",zIndex:3},children:[m?e.jsx("span",{style:{display:"inline-flex",alignItems:"center",gap:"4px",padding:"4px 10px",borderRadius:"999px",background:"linear-gradient(135deg, #ff8a3d, #ff5a1f)",color:"#fff",fontSize:"0.68rem",fontWeight:"900",letterSpacing:"0.06em",textTransform:"uppercase",boxShadow:"0 4px 14px rgba(255, 90, 31, 0.45)"},children:"★ MAIN HEADLINER"}):e.jsx("span",{style:{padding:"3px 9px",borderRadius:"999px",background:"rgba(0, 0, 0, 0.65)",backdropFilter:"blur(8px)",color:"#f6eee8",fontSize:"0.66rem",fontWeight:"700",border:"1px solid rgba(255,255,255,0.12)"},children:s.stage}),e.jsxs("div",{style:{display:"flex",alignItems:"flex-end",gap:"2px",height:"14px",padding:"3px 6px",borderRadius:"999px",background:"rgba(0, 0, 0, 0.55)",backdropFilter:"blur(6px)"},children:[e.jsx("span",{className:"sss-eq-bar sss-eq-1"}),e.jsx("span",{className:"sss-eq-bar sss-eq-2"}),e.jsx("span",{className:"sss-eq-bar sss-eq-3"})]})]}),e.jsxs("div",{style:{position:"absolute",inset:0,background:"linear-gradient(180deg, rgba(0,0,0,0.08) 0%, transparent 40%, rgba(10,5,3,0.72) 65%, rgba(6,3,2,0.96) 100%)",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"16px 14px",zIndex:2},children:[e.jsx("span",{style:{display:"inline-block",width:"fit-content",padding:"2px 8px",borderRadius:"999px",background:"rgba(255, 138, 61, 0.18)",border:"1px solid rgba(255, 138, 61, 0.35)",color:"#ffbd59",fontSize:"0.68rem",fontWeight:"800",letterSpacing:"0.04em",textTransform:"uppercase",marginBottom:"6px"},children:s.genre}),e.jsx("h3",{style:{margin:0,color:"#ffffff",fontSize:"1.28rem",fontWeight:"900",letterSpacing:"-0.02em",lineHeight:1.15,textTransform:"uppercase",textShadow:"0 2px 10px rgba(0,0,0,0.7)"},children:s.name}),e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"8px",paddingTop:"6px",borderTop:"1px solid rgba(255,255,255,0.1)",color:"rgba(255, 255, 255, 0.72)",fontSize:"0.68rem",fontWeight:"700"},children:[e.jsxs("span",{children:["⚡ ",s.bpm]}),e.jsxs("span",{children:["♫ ",s.vibe]})]})]})]},s.name)})}),e.jsxs("div",{style:{display:"flex",alignItems:"center",justifyContent:"center",gap:"1rem",marginTop:"0.8rem",zIndex:10},children:[e.jsx("button",{type:"button",onClick:()=>se("left"),disabled:g===0,"aria-label":"Previous artist",style:{width:"40px",height:"40px",borderRadius:"50%",border:g===0?"1px solid rgba(255, 255, 255, 0.1)":"1px solid rgba(255, 138, 61, 0.35)",background:g===0?"rgba(10, 5, 3, 0.4)":"rgba(20, 10, 6, 0.75)",backdropFilter:"blur(10px)",color:g===0?"rgba(255, 255, 255, 0.25)":"#ffbd59",cursor:g===0?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.2rem",boxShadow:"0 4px 20px rgba(0,0,0,0.5)",transition:"all 200ms ease"},onMouseEnter:s=>{g>0&&(s.currentTarget.style.transform="scale(1.1)",s.currentTarget.style.borderColor="#ff8a3d")},onMouseLeave:s=>{s.currentTarget.style.transform="scale(1)",s.currentTarget.style.borderColor=g===0?"rgba(255, 255, 255, 0.1)":"rgba(255, 138, 61, 0.35)"},children:"‹"}),e.jsx("div",{style:{display:"flex",alignItems:"center",gap:"6px"},children:o.map((s,c)=>e.jsx("span",{onClick:()=>X(c),style:{width:c===g?"20px":"6px",height:"6px",borderRadius:"999px",backgroundColor:c===g?"#ff8a3d":"rgba(255, 255, 255, 0.25)",cursor:"pointer",transition:"all 250ms ease"}},c))}),e.jsx("button",{type:"button",onClick:()=>se("right"),disabled:g===o.length-1,"aria-label":"Next artist",style:{width:"40px",height:"40px",borderRadius:"50%",border:g===o.length-1?"1px solid rgba(255, 255, 255, 0.1)":"1px solid rgba(255, 138, 61, 0.35)",background:g===o.length-1?"rgba(10, 5, 3, 0.4)":"rgba(20, 10, 6, 0.75)",backdropFilter:"blur(10px)",color:g===o.length-1?"rgba(255, 255, 255, 0.25)":"#ffbd59",cursor:g===o.length-1?"default":"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.2rem",boxShadow:"0 4px 20px rgba(0,0,0,0.5)",transition:"all 200ms ease"},onMouseEnter:s=>{g<o.length-1&&(s.currentTarget.style.transform="scale(1.1)",s.currentTarget.style.borderColor="#ff8a3d")},onMouseLeave:s=>{s.currentTarget.style.transform="scale(1)",s.currentTarget.style.borderColor=g===o.length-1?"rgba(255, 255, 255, 0.1)":"rgba(255, 138, 61, 0.35)"},children:"›"})]}),e.jsx("style",{children:`
        .sss-eq-bar {
          width: 2px;
          border-radius: 999px;
          background: #ff8a3d;
      ...
      `})]})}const pe={color:"#ff8a3d",rounded:24,thickness:2.5,borderSize:45,glow:85,movement:"continuous",speed:16},mt=2,Ue=[{blur:8,opacity:.5,reach:.3},{blur:15,opacity:.3,reach:.6},{blur:40,opacity:.18,reach:1}],ft=Math.max(...Ue.map(r=>r.blur)),Me=36;function we(r,n){const t=Math.max(0,Math.min(1,n));if(typeof r!="string")return`rgba(0,0,0,${t})`;const i=r.trim(),l=i.match(/^#([0-9a-f]{3,8})$/i);if(l){let w=l[1];(w.length===3||w.length===4)&&(w=w.split("").map(v=>v+v).join(""));const E=parseInt(w.slice(0,6),16);return Number.isFinite(E)?`rgba(${E>>16&255},${E>>8&255},${E&255},${t})`:`rgba(0,0,0,${t})`}const y=i.match(/^rgba?\(([^)]+)\)/i);if(y){const w=y[1].split(",").map(E=>parseFloat(E));if(w.length>=3&&w.slice(0,3).every(Number.isFinite))return`rgba(${w[0]},${w[1]},${w[2]},${t})`}return`rgba(0,0,0,${t})`}function gt(r,n,t){const i=(r%1+1)%1*2*(n+t);return i<n?[i,0]:i<n+t?[n,i-n]:i<n*2+t?[n-(i-n-t),t]:[0,t-(i-n*2-t)]}function Ce(r,n,t){const i=2*(n+t),l=[0,n/i,(n+t)/i,(n*2+t)/i];return Math.floor(r/4)+l[(r%4+4)%4]}function ht(r,n,t){const[i,l]=gt(r,n,t);return Math.atan2(i-n/2,t/2-l)*180/Math.PI}const De=24,ut=.015;function je(r,n,t,i,l){const y=t>0?t:100,w=i>0?i:100,E=Math.max(0,Math.min(100,n)),v=Math.max(ut,E/100*.5),R=E/100,D=[];let L=0,B=0,H=0;for(let O=0;O<=De;O++){const _=O/De,P=ht(r+(_-.5)*v,y,w);if(O===0)L=P;else{let z=P-B;for(;z>180;)z-=360;for(;z<-180;)z+=360;H+=z}B=P;const V=Math.abs(_-.5)*2,T=R>=1||V<=R?1:1-(V-R)/(1-R);D.push(`${we(l,T*T*(3-2*T))} ${H.toFixed(2)}deg`)}const U=H.toFixed(2);return D.push(`${we(l,0)} ${U}deg`),D.push(`${we(l,0)} 360deg`),`conic-gradient(from ${L.toFixed(2)}deg at 50% 50%, ${D.join(", ")})`}const Le=30,bt=4,Be=3,xt=.35,yt=[.72,.16,.18,1.05],vt=[.65,0,.35,1];function _e(r){const[n,t,i,l]=r;if(n===t&&i===l)return w=>w;const y=(w,E,v)=>{const R=1-v;return 3*R*R*v*w+3*R*v*v*E+v*v*v};return w=>{const E=Math.max(0,Math.min(1,w));let v=E;for(let R=0;R<8;R++){const D=y(n,i,v)-E,L=1-v,B=3*L*L*n+6*L*v*(i-n)+3*v*v*(1-i);if(Math.abs(B)<1e-6)break;v-=D/B,v=Math.max(0,Math.min(1,v))}return y(t,l,v)}}const wt=_e(yt),jt=_e(vt),Oe={WebkitMaskImage:"linear-gradient(#fff 0 0), linear-gradient(#fff 0 0)",WebkitMaskClip:"content-box, border-box",WebkitMaskComposite:"xor",maskImage:"linear-gradient(#fff 0 0), linear-gradient(#fff 0 0)",maskClip:"content-box, border-box",maskComposite:"exclude"};function ae({color:r=pe.color,rounded:n=pe.rounded,thickness:t=pe.thickness,borderSize:i=pe.borderSize,glow:l=pe.glow,movement:y=pe.movement,speed:w=pe.speed,style:E,className:v=""}){const R=u.useRef(null),D=u.useRef(null),L=u.useRef({speed:w,movement:y,borderSize:i,color:r});L.current={speed:w,movement:y,borderSize:i,color:r};const B=u.useRef(null),H=u.useRef({w:0,h:0}),[U,O]=u.useState({w:0,h:0});u.useEffect(()=>{const d=B.current;if(!d||typeof ResizeObserver>"u")return;const a=new ResizeObserver(()=>{const o=d.getBoundingClientRect();o.width===H.current.w&&o.height===H.current.h||(H.current={w:o.width,h:o.height},O(H.current))});return a.observe(d),()=>a.disconnect()},[]),u.useEffect(()=>{const d=B.current;let a=!0,o=null;typeof IntersectionObserver<"u"&&d&&(o=new IntersectionObserver(G=>{G[0]&&(a=G[0].isIntersecting)},{rootMargin:"100px"}),o.observe(d));let x=0,b=performance.now(),M=0,N=0,K=0;const F=G=>{if(x=requestAnimationFrame(F),!a||typeof document<"u"&&document.hidden){b=G;return}const J=Math.min(.05,Math.max(0,(G-b)/1e3));b=G;const W=L.current,C=Math.max(0,Math.min(20,W.speed));if(C>0){const X=W.movement==="step",se=X?Be+(xt-Be)*(C-1)/19:(Le+(bt-Le)*(C-1)/19)/4;for(K+=J/se;K>=1;)K-=1,N+=1;const s=X?wt(Math.min(1,K*2)):jt(K),{w:c,h:m}=H.current,S=c>0?c:100,p=m>0?m:100,$=Ce(N,S,p),k=Ce(N+1,S,p);M=$+(k-$)*s;const Z=R.current;Z&&Z.style.setProperty("--arc",je(M,W.borderSize,c,m,W.color));const ee=D.current;ee&&ee.style.setProperty("--arc",je(M+.5,W.borderSize,c,m,W.color))}};return x=requestAnimationFrame(F),()=>{cancelAnimationFrame(x),o&&o.disconnect()}},[]);const _=Math.max(1,Math.min(10,t)),P=Math.max(0,Math.min(100,n))/100*(Math.min(U.w||200,U.h||200)/2),V=Math.max(0,Math.min(100,l))/100,T=d=>_+V*Me*d,z=10+Me+ft*2,ie=(d,a=0)=>e.jsx("div",{style:{position:"absolute",inset:a-d,boxSizing:"border-box",padding:d,borderRadius:P>0?P+d:0,background:"var(--arc)",...Oe}}),re=(d,a,o,x)=>e.jsx("div",{style:{position:"absolute",inset:-z,boxSizing:"border-box",padding:z,borderRadius:P>0?P+z:0,opacity:x,mixBlendMode:"plus-lighter",filter:o?`blur(${o.toFixed(1)}px)`:"none",WebkitFilter:o?`blur(${o.toFixed(1)}px)`:"none",...Oe},children:ie(a,z)},d),g=(d,a)=>e.jsxs("div",{ref:a,style:{position:"absolute",inset:0,overflow:"visible",pointerEvents:"none","--arc":je(d,i,U.w,U.h,r)},children:[V>0&&Ue.map((o,x)=>re(`glow-${x}`,T(o.reach),o.blur,o.opacity)),Array.from({length:mt}).map((o,x)=>e.jsx("div",{style:{position:"absolute",inset:0,mixBlendMode:"plus-lighter"},children:ie(_)},`edge-${x}`))]});return e.jsxs("div",{ref:B,className:`neon-border-overlay ${v}`,style:{position:"absolute",inset:0,width:"100%",height:"100%",flexShrink:0,borderRadius:P,pointerEvents:"none",zIndex:1,...E},children:[g(0,R),g(.5,D)]})}const Nt=1.5,St=42,At=Math.PI*2,It=300,Et=300,Pt=2,Rt=5e3,kt=3.2,Tt=.05,zt=3,Ft=1.35,Mt=.08,Ct=.5,Dt=.24,Lt=.35,Bt=At*.75,Ot=2,We=3,xe=720,Ht=`
...
void main() {
    ...
    float halfW  = uThick * 0.5 * ${We.toFixed(1)} * near;
