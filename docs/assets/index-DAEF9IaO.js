...
`;function wt(t,r,n){const a=t.createShader(r);return a?(t.shaderSource(a,n),t.compileShader(a),t.getShaderParameter(a,t.COMPILE_STATUS)?a:(console.error("ParticleDrift shader:",t.getShaderInfoLog(a)),t.deleteShader(a),null)):null}function At(t,r,n){const a=wt(t,t.VERTEX_SHADER,r),o=wt(t,t.FRAGMENT_SHADER,n);if(!a||!o)return null;const i=t.createProgram();return i?(t.attachShader(i,a),t.attachShader(i,o),t.linkProgram(i),t.getProgramParameter(i,t.LINK_STATUS)?i:(console.error("ParticleDrift link:",t.getProgramInfoLog(i)),null)):null}function Nt(t,r){if(!t)return r;const n=String(t).trim();if(n.charAt(0)==="#"){let o=n.slice(1);if((o.length===3||o.length===4)&&(o=o[0]+o[0]+o[1]+o[1]+o[2]+o[2]),o.length>=6){const i=parseInt(o.slice(0,2),16),l=parseInt(o.slice(2,4),16),c=parseInt(o.slice(4,6),16);if(!isNaN(i)&&!isNaN(l)&&!isNaN(c))return[i/255,l/255,c/255]}return r}const a=n.match(/[\d.]+/g);return a&&a.length>=3?[Math.min(255,parseFloat(a[0]))/255,Math.min(255,parseFloat(a[1]))/255,Math.min(255,parseFloat(a[2]))/255]:r}function ie(t,r){return typeof t=="number"&&isFinite(t)?t:r}function Re(t,r,n){return t<r?r:t>n?n:t}function Ls(t){let r=t>>>0;return function(){r+=1831565813;let n=r;return n=Math.imul(n^n>>>15,n|1),n^=n+Math.imul(n^n>>>7,n|61),((n^n>>>14)>>>0)/4294967296}}const ct=[[0,-1],[1,-1],[1,1],[0,-1],[1,1],[0,1]];function Ps({style:t,background:r="transparent",baseColor:n="#F84400",accentColor:a="#ECECEC",density:o=235,dotSize:i=14,speed:l=100,direction:c=0,hover:d=200,linkDistance:u=202,linkThickness:h=2.5,width:x,height:g,isFixed:k=!0}){const Ae=m.useRef(null),b=m.useRef(null),$=m.useRef({w:0,h:0});$.current={w:ie(x,0),h:ie(g,0)};const w=m.useRef({x:-1e4,y:-1e4}),Ne=m.useRef({});Ne.current={base:n,accent:a,density:Math.round(Re(ie(o,90),10,400)),dotSize:Re(ie(i,5),1,24),speed:Re(ie(l,50),0,100)/50,direction:Re(ie(c,0),0,360),hover:Re(ie(d,100),0,200)/100,linkDistance:Re(ie(u,120),0,400),linkThickness:Re(ie(h,1),.5,8)},m.useEffect(()=>{const y=b.current;if(!y)return;const s=y.getContext("webgl",{alpha:!0,antialias:!1,depth:!1,premultipliedAlpha:!0});if(!s){console.error("ParticleDrift: WebGL unavailable");return}const z=At(s,Es,_s),D=At(s,Rs,Ss);if(!z||!D)return;const Le=new Map,V=(A,N)=>{const j=(A===z?"L:":"D:")+N;return Le.has(j)||Le.set(j,s.getUniformLocation(A,N)),Le.get(j)},le=new Float32Array(ke*6*2),de=new Float32Array(ke*6*2),ue=new Float32Array(ke*6*2),he=new Float32Array(ke*6*3);for(let A=0;A<ke;A++)for(let N=0;N<6;N++){const j=(A*6+N)*2;ue[j]=ct[N][0],ue[j+1]=ct[N][1]}const U=s.createBuffer(),te=s.createBuffer(),me=s.createBuffer(),pe=s.createBuffer();s.bindBuffer(s.ARRAY_BUFFER,U),s.bufferData(s.ARRAY_BUFFER,le.byteLength,s.DYNAMIC_DRAW),s.bindBuffer(s.ARRAY_BUFFER,te),s.bufferData(s.ARRAY_BUFFER,de.byteLength,s.DYNAMIC_DRAW),s.bindBuffer(s.ARRAY_BUFFER,me),s.bufferData(s.ARRAY_BUFFER,ue,s.STATIC_DRAW),s.bindBuffer(s.ARRAY_BUFFER,pe),s.bufferData(s.ARRAY_BUFFER,he.byteLength,s.DYNAMIC_DRAW);const se=Ls(20260824);let re=0,_=new Float32Array(0),R=new Float32Array(0),Ee=new Float32Array(0),W=new Float32Array(0),fe=new Float32Array(0);const P=s.createBuffer(),f=s.createBuffer(),xe=(A,N,j)=>{re=A,_=new Float32Array(A),R=new Float32Array(A),Ee=new Float32Array(A),W=new Float32Array(A*2),fe=new Float32Array(A);for(let J=0;J<A;J++)_[J]=se()*N,R[J]=se()*j,Ee[J]=(se()*.4+.1)*60;s.bindBuffer(s.ARRAY_BUFFER,P),s.bufferData(s.ARRAY_BUFFER,W.byteLength,s.DYNAMIC_DRAW),s.bindBuffer(s.ARRAY_BUFFER,f),s.bufferData(s.ARRAY_BUFFER,fe.byteLength,s.DYNAMIC_DRAW)};let K=0,Z=performance.now(),ge=-1,ne=0,H=0;const _e=A=>{const N=Math.min(.05,(A-Z)/1e3);Z=A;const j=Ne.current,J=j.speed,Ke=Math.min(window.devicePixelRatio||1,Ns),F=$.current.w||y.clientWidth||window.innerWidth||1200,Y=$.current.h||y.clientHeight||window.innerHeight||800,Ze=Math.max(1,Math.round(F*Ke)),Je=Math.max(1,Math.round(Y*Ke));(y.width!==Ze||y.height!==Je)&&(y.width=Ze,y.height=Je),s.viewport(0,0,Ze,Je);const Qe=F<768,Mt=F>=768&&F<1024;let ze=j.density,et=j.linkDistance,tt=j.dotSize;if(Qe?(ze=Math.max(60,Math.min(Math.round(j.density*.38),90)),et=Math.min(j.linkDistance,135),tt=Math.min(j.dotSize,8.5)):Mt&&(ze=Math.max(90,Math.min(Math.round(j.density*.6),145)),et=Math.min(j.linkDistance,165),tt=Math.min(j.dotSize,11)),ze!==ge&&(xe(ze,F,Y),ge=ze),F!==ne||Y!==H){const p=F/Math.max(ne||F,1),E=Y/Math.max(H||Y,1);for(let M=0;M<re;M++)_[M]*=p,R[M]*=E;ne=F,H=Y}const Ye=w.current,mt=j.hover,st=(Qe?100:180)*(mt>0?1:0),be=et,pt=Qe?Math.min(j.linkThickness,1.6):j.linkThickness,ft=j.direction*Math.PI/180,Tt=Math.sin(ft),Ft=Math.cos(ft);let ve=0;const xt=(p,E,M,G,O,ae,Oe,$e)=>{if(!(ve>=ke)){for(let je=0;je<6;je++){const We=(ve*6+je)*2,rt=(ve*6+je)*3;le[We]=p,le[We+1]=E,de[We]=M,de[We+1]=G,he[rt]=ct[je][0]===0?O:ae,he[rt+1]=Oe,he[rt+2]=$e}ve++}};for(let p=0;p<re;p++){_[p]+=Ee[p]*Tt*N*J,R[p]+=Ee[p]*Ft*N*J,_[p]<-we?(_[p]=F+we,R[p]=se()*Y):_[p]>F+we&&(_[p]=-we,R[p]=se()*Y),R[p]<-we?(R[p]=Y+we,_[p]=se()*F):R[p]>Y+we&&(R[p]=-we,_[p]=se()*F);const E=Ye.x-_[p],M=Ye.y-R[p],G=Math.sqrt(E*E+M*M),O=st>0&&G<st?1:0;if(O===1){const ae=.5*(1-G/st)*mt;xt(_[p],R[p],Ye.x,Ye.y,ae,ae,1,pt)}W[p*2]=_[p],W[p*2+1]=R[p],fe[p]=O}if(be>0){const p=be*be;for(let E=0;E<re&&ve<ke;E++){const M=_[E],G=R[E];for(let O=E+1;O<re&&ve<ke;O++){const ae=M-_[O];if(ae>be||ae<-be)continue;const Oe=G-R[O];if(Oe>be||Oe<-be)continue;const $e=ae*ae+Oe*Oe;if($e>=p)continue;const je=.15*(1-Math.sqrt($e)/be);xt(M,G,_[O],R[O],je,je,0,pt)}}}s.clearColor(0,0,0,0),s.clear(s.COLOR_BUFFER_BIT),s.disable(s.DEPTH_TEST),s.enable(s.BLEND),s.blendFunc(s.ONE,s.ONE_MINUS_SRC_ALPHA);const Te=Nt(j.base,[.973,.267,0]),Fe=Nt(j.accent,[.925,.925,.925]);if(ve>0){s.useProgram(z);const p=ve*6;s.bindBuffer(s.ARRAY_BUFFER,U),s.bufferSubData(s.ARRAY_BUFFER,0,le.subarray(0,p*2));const E=s.getAttribLocation(z,"a_p0");s.enableVertexAttribArray(E),s.vertexAttribPointer(E,2,s.FLOAT,!1,0,0),s.bindBuffer(s.ARRAY_BUFFER,te),s.bufferSubData(s.ARRAY_BUFFER,0,de.subarray(0,p*2));const M=s.getAttribLocation(z,"a_p1");s.enableVertexAttribArray(M),s.vertexAttribPointer(M,2,s.FLOAT,!1,0,0),s.bindBuffer(s.ARRAY_BUFFER,me);const G=s.getAttribLocation(z,"a_corner");s.enableVertexAttribArray(G),s.vertexAttribPointer(G,2,s.FLOAT,!1,0,0),s.bindBuffer(s.ARRAY_BUFFER,pe),s.bufferSubData(s.ARRAY_BUFFER,0,he.subarray(0,p*3));const O=s.getAttribLocation(z,"a_shade");s.enableVertexAttribArray(O),s.vertexAttribPointer(O,3,s.FLOAT,!1,0,0),s.uniform2f(V(z,"uSize"),F,Y),s.uniform3f(V(z,"uBase"),Te[0],Te[1],Te[2]),s.uniform3f(V(z,"uAccent"),Fe[0],Fe[1],Fe[2]),s.drawArrays(s.TRIANGLES,0,p),s.disableVertexAttribArray(E),s.disableVertexAttribArray(M),s.disableVertexAttribArray(G),s.disableVertexAttribArray(O)}if(re>0){s.useProgram(D),s.bindBuffer(s.ARRAY_BUFFER,P),s.bufferSubData(s.ARRAY_BUFFER,0,W);const p=s.getAttribLocation(D,"a_pos");s.enableVertexAttribArray(p),s.vertexAttribPointer(p,2,s.FLOAT,!1,0,0),s.bindBuffer(s.ARRAY_BUFFER,f),s.bufferSubData(s.ARRAY_BUFFER,0,fe);const E=s.getAttribLocation(D,"a_lit");s.enableVertexAttribArray(E),s.vertexAttribPointer(E,1,s.FLOAT,!1,0,0),s.uniform2f(V(D,"uSize"),F,Y),s.uniform1f(V(D,"uDpr"),Ke),s.uniform1f(V(D,"uDot"),tt),s.uniform1f(V(D,"uRestAlpha"),.4),s.uniform3f(V(D,"uBase"),Te[0],Te[1],Te[2]),s.uniform3f(V(D,"uAccent"),Fe[0],Fe[1],Fe[2]),s.drawArrays(s.POINTS,0,re),s.disableVertexAttribArray(p),s.disableVertexAttribArray(E)}K=requestAnimationFrame(_e)},Pe=A=>{const N=y.getBoundingClientRect();if(N.width<=0||N.height<=0)return;const j=$.current.w||y.clientWidth||window.innerWidth||1200,J=$.current.h||y.clientHeight||window.innerHeight||800;w.current.x=(A.clientX-N.left)/N.width*j,w.current.y=(A.clientY-N.top)/N.height*J},Me=()=>{w.current.x=-1e4,w.current.y=-1e4};return window.addEventListener("pointermove",Pe,{passive:!0}),window.addEventListener("pointerleave",Me,{passive:!0}),K=requestAnimationFrame(_e),()=>{cancelAnimationFrame(K),window.removeEventListener("pointermove",Pe),window.removeEventListener("pointerleave",Me),s.deleteBuffer(U),s.deleteBuffer(te),s.deleteBuffer(me),s.deleteBuffer(pe),s.deleteBuffer(P),s.deleteBuffer(f),s.deleteProgram(z),s.deleteProgram(D)}},[]);const q=k?{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",zIndex:0,pointerEvents:"none",overflow:"hidden",background:r,isolation:"isolate",...t}:{position:"relative",overflow:"hidden",background:r,isolation:"isolate",width:typeof x=="number"&&x>0?x:"100%",height:typeof g=="number"&&g>0?g:"100%",...t};return e.jsx("div",{ref:Ae,style:q,"aria-hidden":"true",children:e.jsx("canvas",{ref:b,style:{position:"absolute",inset:0,width:"100%",height:"100%",display:"block"}})})}function Ms(){const t="/ozillafest/".replace(/\/$/,"");return e.jsx(Ut,{basename:t||void 0,future:{v7_startTransition:!0,v7_relativeSplatPath:!0},children:e.jsxs(Gt,{children:[e.jsx(Ps,{}),e.jsx(As,{})]})})}class Ts extends m.Component{constructor(r){super(r),this.state={error:null}}static getDerivedStateFromError(r){return{error:r}}componentDidCatch(r,n){console.error("Application render failed:",r,n)}render(){return this.state.error?e.jsxs("main",{style:{padding:"32px",fontFamily:"Arial, sans-serif"},children:[e.jsx("h1",{style:{color:"#7c2d12",marginBottom:"12px"},children:"Ozilla Festival could not render"}),e.jsx("p",{style:{color:"#444",marginBottom:"16px"},children:"Check the browser console for the full error details."}),e.jsx("pre",{style:{whiteSpace:"pre-wrap",background:"#fff7ed",border:"1px solid #fed7aa",padding:"16px",borderRadius:"6px",color:"#7c2d12"},children:this.state.error.message})]}):this.props.children}}lt.createRoot(document.getElementById("root")).render(e.jsx(Ue.StrictMode,{children:e.jsx(Ts,{children:e.jsx($t,{clientId:"placeholder-client-id",children:e.jsx(Ms,{})})})}));export{ds as L,es as P,Ge as a,qe as b,B as c,Zt as g,ee as t,ht as u};
